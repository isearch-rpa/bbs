...
public class SearchProcessor {
    ...
    public void search(final HTTPRequestContext context, final HttpServletRequest request, final HttpServletResponse response)
        ...
        if (!Symphonys.getBoolean("es.enabled") && !Symphonys.getBoolean("algolia.enabled") && !Symphonys.getBoolean("jdbc.enabled")) {
        ...
        if (Symphonys.getBoolean("es.enabled")) {
        	...
        	//过滤内部帖子
        	ArrayList<JSONObject> notMustList=new	ArrayList<JSONObject> ();
        	JSONObject mustNot = new JSONObject();
        	JSONObject mustNotString = new JSONObject();
            mustNot.put("match", mustNotString);
        	final JSONObject domain = domainQueryService.getByURI("内部");
            String domainId="";
            if(user==null||(!user.opt("userRole").equals("adminRole")&&!user.opt("userRole").equals("memberRole"))){
            	if(null!=domain){
            		domainId = domain.optString(Keys.OBJECT_ID);
            		Set<String> articleIds=searchQueryService.getarticleIdsByDomainId(domainId);
            		for (String str : articleIds) {
            			mustNot = new JSONObject();
                        mustNotString = new JSONObject();
                        mustNot.put("match", mustNotString);
                        mustNotString.put("oId", str);
                        notMustList.add(mustNot);
					}
            	}
            }
        	
            final JSONObject result = searchQueryService.searchElasticsearch(Article.ARTICLE, keyword, pageNum, pageSize, notMustList);
            ...
            total = hitsResult.optInt("total");
            //total = result.optInt("total");
        }
        
        if (Symphonys.getBoolean("jdbc.enabled")) {
            final JSONObject domain = domainQueryService.getByURI("内部");
            String domainId="";
            if(user==null||(!user.opt("userRole").equals("adminRole")&&!user.opt("userRole").equals("memberRole"))){
            	if(null!=domain){
        	...
        	final JSONObject result = searchQueryService.searchJdbc(domainId,keyword, pageNum, pageSize);
        	if (null == result || 0 != result.optInt("status")) {
        		response.sendError(HttpServletResponse.SC_NOT_FOUND);
