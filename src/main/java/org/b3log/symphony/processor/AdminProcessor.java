...
public class AdminProcessor {
    ...
    public void addUser(final HTTPRequestContext context, final HttpServletRequest request, final HttpServletResponse response)
        ...
        final String userNickname = request.getParameter(UserExt.USER_NICKNAME).trim();
        final String code = request.getParameter("code");

        final boolean nameInvalid = UserRegisterValidation.invalidUserName(userName);
        //final boolean codeInvalid = UserRegisterValidation.invalidCode(code);
        ...
        final boolean userNicknameInvalid = (userNickname==null||"".equals(userNickname)||userNickname.length()>20)?true:false;
        if (nameInvalid || emailInvalid || passwordInvalid ) {
            ...
            if (nameInvalid) {
                ...
                dataModel.put(Keys.MSG, "昵称格式有误（长度 1-20）");
     ...
     * @throws Exception exception
     */
    @RequestProcessing(value = "/admin/user/{userId}/code", method = HTTPRequestMethod.POST)
    @Before(adviceClass = {StopwatchStartAdvice.class, PermissionCheck.class})
    @After(adviceClass = {PermissionGrant.class, StopwatchEndAdvice.class})
    public void updateCode(final HTTPRequestContext context, final HttpServletRequest request, final HttpServletResponse response,
    		final String userId) throws Exception {
    	final JSONObject user = userQueryService.getUser(userId);
    	final String oldCode = user.optString("code");
    	...
    	try {
    		userMgmtService.updateCode(userId, user);
    	} catch (final ServiceException e) {
    		final AbstractFreeMarkerRenderer renderer = new SkinRenderer(request);
    		context.setRenderer(renderer);
    		renderer.setTemplateName("admin/error.ftl");
