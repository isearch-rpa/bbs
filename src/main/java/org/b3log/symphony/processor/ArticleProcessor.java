...
//        	Matcher m = p.matcher((String)article.get("articleContent")); 
//        	while(m.find()) { 
//        		System.out.println(m.group(1));
//        		if (m.group(1).endsWith(".exe")||m.group(1).endsWith(".zip")||m.group(1).endsWith(".rar")||m.group(1).endsWith(".gzip")||m.group(1).endsWith(".jar")||m.group(1).endsWith(".doc")||m.group(1).endsWith(".docx")||m.group(1).endsWith(".xls")||m.group(1).endsWith(".xlsx")||m.group(1).endsWith(".ppt")||m.group(1).endsWith(".dll")||m.group(1).endsWith(".pptx")) {
//        			
//        			article.put("articleContent", ((String)article.get("articleContent")).replace("href=\""+m.group(1)+"\"","onclick=(notAllowDownLoad())"));
//        		}
//        	}
//        	
//        }
    ...
    public void addArticle(final HTTPRequestContext context, final HttpServletRequest request, final JSONObject requestJSONObject) {
        ...
        //如果是内部标签 改为匿名不可访问
        final List<JSONObject> tags = domainQueryService.getTags(Symphonys.get("insideDomainId"));
        if(tags!=null&&articleTags!=null){
	        String[] tags1=articleTags.split(",");
	       
	        for (int i = 0; i < tags.size(); i++) {
	        	try {
					if(Arrays.asList(tags1).contains(tags.get(i).get("tagTitle"))){
				        article.put("articleAnonymousView", 1);
				        break;
    ...
    public void updateArticle(final HTTPRequestContext context, final HttpServletRequest request, final HttpServletResponse response,
        ...
        if(issideDomain){
        	...
        	article.put("articleAnonymousView", 0);
        }
    ...
    @RequestProcessing(value = "/vieo", method = HTTPRequestMethod.GET)
    @Before(adviceClass = {StopwatchStartAdvice.class, AnonymousViewCheck.class})
    ...
    public void showBindDD(final HTTPRequestContext context, final HttpServletRequest request, final HttpServletResponse response)
    		throws Exception {
    	final AbstractFreeMarkerRenderer renderer = new SkinRenderer(request);
    	String vieoId=request.getParameter("vieoId");
    	context.setRenderer(renderer);
    	final Map<String, Object> dataModel = renderer.getDataModel();
    	renderer.setTemplateName("verify/vieo.ftl");
		dataModel.put("vieoId", vieoId);
    	dataModelService.fillHeaderAndFooter(request, response, dataModel);
    }
