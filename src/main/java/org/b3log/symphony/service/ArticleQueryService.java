...
public class ArticleQueryService {
    ...
    public JSONObject getDomainArticles(final int avatarViewMode, final String domainId,
        ...
        try {
            ...
            JSONObject result1 = articleRepository.get(query);
            final List<JSONObject> articles
                    = CollectionUtils.jsonArrayToList(result1.optJSONArray(Keys.RESULTS));

            final int pageCount = result1.optJSONObject(Pagination.PAGINATION).optInt(Pagination.PAGINATION_PAGE_COUNT);
...
//                        final double v = o2.optDouble(Article.ARTICLE_STICK) - o1.optDouble(Article.ARTICLE_STICK);
//                        if (0 == v) {
...
//                    });
    ...
    private CompositeFilter makeRecentArticleShowingFilter1(Set<String> ids) {
    	final List<Filter> filters = new ArrayList<>();
    	filters.add(new PropertyFilter(Article.ARTICLE_STATUS, FilterOperator.NOT_EQUAL, Article.ARTICLE_STATUS_C_INVALID));
    	filters.add(new PropertyFilter(Article.ARTICLE_TYPE, FilterOperator.NOT_EQUAL, Article.ARTICLE_TYPE_C_DISCUSSION));
    	filters.add(new PropertyFilter(Article.ARTICLE_TAGS, FilterOperator.NOT_EQUAL, Tag.TAG_TITLE_C_SANDBOX));
    ...
    private Query makeRecentDefaultQuery1(Set<String> ids,final int currentPageNum, final int fetchSize) {
    	final Query ret = new Query()
    			.addSort(Article.ARTICLE_STICK, SortDirection.DESCENDING)
    			.addSort(Keys.OBJECT_ID, SortDirection.DESCENDING)
    			.setPageSize(fetchSize).setCurrentPageNum(currentPageNum);
    	ret.setFilter(makeRecentArticleShowingFilter1(ids));
    	ret.addProjection(Keys.OBJECT_ID, String.class).
    	addProjection(Article.ARTICLE_STICK, Long.class).
    	addProjection(Article.ARTICLE_CREATE_TIME, Long.class).
    	addProjection(Article.ARTICLE_UPDATE_TIME, Long.class).
    ...
    private Query makeRecentHotQuery1(Set<String> ids,final int currentPageNum, final int fetchSize) {
    	final String id = String.valueOf(DateUtils.addMonths(new Date(), -1).getTime());
    ...
    private Query makeRecentGoodQuery1(final Set<String> ids,final int currentPageNum, final int fetchSize) {
    	final Query ret = new Query()
    			.addSort(Article.ARTICLE_STICK, SortDirection.DESCENDING)
    			.addSort(Article.REDDIT_SCORE, SortDirection.DESCENDING)
    			.addSort(Keys.OBJECT_ID, SortDirection.DESCENDING)
    ...
    private Query makeRecentReplyQuery1(final Set<String> ids,final int currentPageNum, final int fetchSize) {
    	final Query ret = new Query()
    			.addSort(Article.ARTICLE_STICK, SortDirection.DESCENDING)
    			.addSort(Article.ARTICLE_LATEST_CMT_TIME, SortDirection.DESCENDING)
    			.addSort(Keys.OBJECT_ID, SortDirection.DESCENDING)
    ...
    public JSONObject getRecentArticles1(final String domainId,final int avatarViewMode, final int sortMode,
    		final int currentPageNum, final int fetchSize)
    				throws ServiceException {
    	final JSONObject ret = new JSONObject();
    	...
    	try {
    		...
    		switch (sortMode) {
        	case 0:
        		query = makeRecentDefaultQuery1(articleIds,currentPageNum, fetchSize);
//        		query = makeRecentDefaultQuery(currentPageNum, fetchSize);
        		
        		break;
        	case 1:
        		query = makeRecentHotQuery1(articleIds,currentPageNum, fetchSize);
//        		query = makeRecentHotQuery(currentPageNum, fetchSize);
        		
        		break;
        	case 2:
        		query = makeRecentGoodQuery1(articleIds,currentPageNum, fetchSize);
//        		query = makeRecentGoodQuery(articleIds,currentPageNum, fetchSize);
        		
        		break;
        	case 3:
        		query = makeRecentReplyQuery1(articleIds,currentPageNum, fetchSize);
//        		query = makeRecentReplyQuery(currentPageNum, fetchSize);
     ...
     * @throws ServiceException service exception
     */
    public List<JSONObject> getIndexRecentArticles(final String isIn,final int avatarViewMode) throws ServiceException {
        try {
            List<JSONObject> ret;
            ...
            try {
            	if("true".equals(isIn)){
            		ret = articleRepository.select("SELECT\n"
                            ...
                            + " WHERE oId not in (SELECT article_oId  from symphony_tag_article where tag_oId in (select tag_oid from symphony_domain_tag where domain_oId=(select oId from symphony_domain where domainURI='内部'))) and `articleType` != 1 AND `articleStatus` = 0 AND `articleTags` != '" + Tag.TAG_TITLE_C_SANDBOX + "' AND `oId` >= '" + thirtyDaysAgo + "'\n"
                            + " ORDER BY\n"
                            + "	articleStick DESC,\n"
                            + "	flag DESC\n"
...
//            genParticipants(ret, participantsCnt);
     ...
     * @throws ServiceException service exception
     */
    public JSONObject getPerfectArticles(final String domainId,final int avatarViewMode, final int currentPageNum, final int fetchSize)
            throws ServiceException {
    ...
    public String getArticleMetaDesc(final JSONObject article) {
        ...
        try {
            ...
            final int length = Integer.valueOf("120");
//            final int length = Integer.valueOf("150");
