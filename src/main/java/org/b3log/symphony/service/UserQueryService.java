...
public class UserQueryService {
    ...
    public JSONObject getByDDId(final String ddId) throws ServiceException {
    	try {
    		return userRepository.getByDDId(ddId);
    	} catch (final RepositoryException e) {
    		LOGGER.log(Level.ERROR, "Gets user by ddId[" + ddId + "] failed", e);
    		throw new ServiceException(e);
    	}
    }
    ...
    public JSONObject getUserByCode(final String code) {
    	try {
    		final JSONObject ret = userRepository.getByCode(code);
    		if (null == ret) {
    			return null;
    		}
    		...
    		LOGGER.log(Level.ERROR, "Gets user by code[" + code + "] failed", e);
    		
    		return null;
    	}
    }
    public JSONObject getByCodeAndUserName(final String userName,final String code) {
    	try {
    		final JSONObject ret = userRepository.getByCodeAndUserName(userName,code);
    		if (null == ret) {
    			return null;
    		}
    ...
    public JSONObject getByRoleAndEmail(final String role,final String email) {
    	try {
    		final JSONObject ret = userRepository.getByRoleAndEmail(role,email);
    		if (null == ret) {
    			return null;
    		}
    		...
    		LOGGER.log(Level.ERROR, "Gets user RoleAndEmail failed", e);
    ...
    public JSONObject getByRoleAndUserName(final String role,final String userName) {
    	try {
    		final JSONObject ret = userRepository.getByRoleAndUserName(role,userName);
    		if (null == ret) {
    			return null;
    		}
    ...
    public JSONObject getUsers(final JSONObject requestJSONObject) throws ServiceException {
        ...
        if (requestJSONObject.has(Common.QUERY)) {
            ...
            filters.add(new PropertyFilter(User.USER_EMAIL, FilterOperator.LIKE, "%"+q+"%"));
            filters.add(new PropertyFilter(Keys.OBJECT_ID, FilterOperator.EQUAL, q));
            filters.add(new PropertyFilter("userNickname", FilterOperator.LIKE, "%"+q+"%"));
