...
public class DataModelService {
    ...
    public void fillSideHotArticles(final Map<String, Object> dataModel) {
        ...
        try {
			...
			try {
				//添加优选模块
				final JSONObject insideDomain = domainQueryService.getByURI("内部");
				String insideDomainId="";
				insideDomainId = insideDomain.optString(Keys.OBJECT_ID);
				JSONObject result1;
				result1 = articleQueryService.getPerfectArticles(insideDomainId,0, 1, Symphonys.getInt("perfectArticlesCnt"));
				final List<JSONObject> perfectArticles = (List<JSONObject>) result1.get(Article.ARTICLES);
				dataModel.put("prefectArticles", perfectArticles);
			} catch (Exception e) {
    ...
    private void fillHeader(final HttpServletRequest request, final HttpServletResponse response,
        ...
        dataModel.put("jdbcEnabled", Symphonys.getBoolean("jdbc.enabled"));
