...
public class UserMgmtService {
    ...
    public synchronized String addUser(final JSONObject requestJSONObject) throws ServiceException {
        ...
        try {
            ...
            final String code = requestJSONObject.optString("code");
            final String ddId = requestJSONObject.optString("ddId");
...
//            user = userRepository.getByCode(code);
//            if (null != user) {
...
//                    }
//
//                    throw new ServiceException("机器码重复[" + code + "]");
//                }
//            }
    ...
    public synchronized String symAddUser(final JSONObject requestJSONObject) throws ServiceException {
    	final Transaction transaction = userRepository.beginTransaction();
...
//            }
    ...
    /**
     * Updates the specified user's code by the given user id.
     *
     * @param userId the given user id
     * @param user   the specified user, contains the new code
     * @throws ServiceException service exception
     */
    public void updateCode(final String userId, final JSONObject user) throws ServiceException {
    	final String newCode = user.optString("code");
    	
    	final Transaction transaction = userRepository.beginTransaction();
    	
    	try {
    		if (newCode==null||"".equals(newCode)) {
    			throw new ServiceException( "机器码不能为空");
    		}
    		if (null != userRepository.getByCode(newCode)) {
    			throw new ServiceException( "机器码重复 [" + newCode + "]");
    		}
     ...
     */
    public void updateDDId(final String userId, final JSONObject user) throws ServiceException {
    	final String ddId = user.optString("ddId");
    	
    	final Transaction transaction = userRepository.beginTransaction();
    	
    	try {
    		if (ddId==null||"".equals(ddId)) {
    			throw new ServiceException( "钉钉id不能为空");
    		}
    		userRepository.update(userId, user);
    		...
    		LOGGER.log(Level.ERROR, "Updates ddId of the user[id=" + ddId + "] failed", e);
    		throw new ServiceException(e);
    	}
    }
