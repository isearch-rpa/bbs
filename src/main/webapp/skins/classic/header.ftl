...
-->
<div class="nav">
    <h1 aria-label="Rpa" class="tooltipped tooltipped-s">
        <a href="${servePath}">
            <#--<svg><use xlink:href="#logo"></use></svg>-->
            <img src="${staticServePath}/images/logo6.png"  style="margin-top:5px" />
        </a>
    </h1>
    <div class="nav-tabs">
        <#list domains as domain>
	        <#if domain.domainTitle=="内部">
	        	<#if isLoggedIn>
	        		<#if currentUser.userRole=="memberRole" || currentUser.userRole=="adminRole">
				        <a pjax-title="${domain.domainTitle} - ${domainLabel} - Rpa" href="${servePath}/domain/${domain.domainURI}"<#if selected?? && selected == domain.domainURI> class="current"</#if>>${domain.domainIconPath} ${domain.domainTitle}</a>&nbsp;&nbsp;
	        		</#if>
	        	</#if>
	        <#else>
			        <a pjax-title="${domain.domainTitle} - ${domainLabel} - Rpa" href="${servePath}/domain/${domain.domainURI}"<#if selected?? && selected == domain.domainURI> class="current"</#if>>${domain.domainIconPath} ${domain.domainTitle}</a>
        ...
        <a pjax-title="${latestLabel} - Rpa" href="${servePath}/recent"<#if selected?? && 'recent' == selected> class="current"</#if>>
    ...
    <#if esEnabled || algoliaEnabled || jdbcEnabled>
    <form class="responsive-hide fn-left" target="_self" action="${servePath}/search">
        <input class="search" placeholder="Search RPA" type="text" name="key" id="search" value="<#if key??>${key}</#if>" >
    ...
    <div class="user-nav">
        <#if isLoggedIn>
            ...
            <div class="module person-list" id="personListPanel">
                <ul>
                    ...
                    <li>
                        <a href="${servePath}/showBindDD"">绑定钉钉</a>
                    </li>
                    <li>
        ...
        <#else>
            <a href="${servePath}/showDDLogin" class="unlogin">${loginLabel}</a>
